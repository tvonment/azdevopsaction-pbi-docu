...
let
    ...
    #"Added Custom1" = Table.AddColumn(#"Filtered Rows", "project_id", each if Text.Contains([CIID], "-") then Text.BeforeDelimiter([CIID], "-") else "", Text.Type),
    #"Added Custom2" = Table.AddColumn(#"Added Custom1", "project_task_id", each if Text.Contains ([CIID], "-") then Text.BeforeDelimiter([CIID], "-", 1) else "", Text.Type),
