...
```dax
Ends on next day = DATEDIFF( DailyTimeRecording[From] , DailyTimeRecording[To],DAY)
```



```dax
is<12h = LOOKUPVALUE('Q3 DidYouWorkLessThan12Hours'[Q3 Value], 'Q3 DidYouWorkLessThan12Hours'[Reference Date], DailyTimeRecording[Reference Date], 'Q3 DidYouWorkLessThan12Hours'[Mail], DailyTimeRecording[Mail]) <> 0
```
...
```dax
count weekend red employees = calculate([# distinct employees],   coalesce('Values by Staffing'[Q2], -1) = 0)
```



```dax
Share of weekend red cases = 

var _weeks = calculatetable(values('Calendar'[ShiftedWeekOffest]))
return AVERAGEX( _weeks,  
    var _red =  Coalesce([count weekend red employees],0)
    var _part = [CountParticipant]
     return divide (_red, _part, blank())
)
```
...
```dax
Share of weekend red cases 4 week average = 
var _weeks = calculatetable(values('Calendar'[ShiftedWeekOffest]), all('Calendar'), 'Calendar'[ShiftedWeekOffest] <= 0 && 'Calendar'[ShiftedWeekOffest] > -4)  
return AVERAGEX( _weeks,   [Share of weekend red cases])
...
```dax
Share of weekend red cases 8 week average = 
var _weeks = calculatetable(values('Calendar'[ShiftedWeekOffest]), all('Calendar'), 'Calendar'[ShiftedWeekOffest] <= 0 && 'Calendar'[ShiftedWeekOffest] > -8)  
return AVERAGEX( _weeks,   [Share of weekend red cases])
```
...
```dax
Share of weekend red cases 12 week average = 
var _weeks = calculatetable(values('Calendar'[ShiftedWeekOffest]), all('Calendar'), 'Calendar'[ShiftedWeekOffest] <= 0 && 'Calendar'[ShiftedWeekOffest] > -12)  
return AVERAGEX( _weeks,   [Share of weekend red cases])
```
...
```dax
count weekend red employees (multiple projects) = 
 var _sum =
  CALCULATETABLE(
    SUMMARIZE( 'Values by Staffing',  'Values by Staffing'[emp_id],  "_dProjectCount", DISTINCTCOUNT('Values by Staffing'[ProjectNumber]))
  , REMOVEFILTERS(Projetcs[Project])
)
var _emp_with_multiple_projects = Filter(_sum
  , [_dProjectCount] > 1)

 return 
  calculate([count weekend red employees], _emp_with_multiple_projects )
...
```dax
share weekend red (multiple projects) = 
var _weeks = calculatetable(values('Calendar'[ShiftedWeekOffest]))
return 
coalesce(AVERAGEX( _weeks,  
    divide(coalesce( [count weekend red employees (multiple projects)],0), [count weekend red employees], Blank())
), 0)
...
```dax
count weekend red employees (ic hours) = 
var _param_hours = [More than IC weekend hours]
var _emp_with_ore_ic_hours = calculatetable(values('Values by Staffing'[emp_id]), 'Values by Staffing'[Q7 Hours] >= _param_hours) 
return calculate([count weekend red employees], _emp_with_ore_ic_hours)
```



```dax
share weekend red (IC hours) = 
var _weeks = calculatetable(values('Calendar'[ShiftedWeekOffest]))
return 
Coalesce(AVERAGEX( _weeks,  
    divide(coalesce([count weekend red employees (ic hours)], 0), [count weekend red employees], Blank())
),0)
```
...
```dax
Share of weekend red cases 4 vs 8 = 
[Share of weekend red cases 4 week average] - [Share of weekend red cases 8 week average]
```



```dax
Share of weekend red cases 4 vs 12 = 
[Share of weekend red cases 4 week average] - [Share of weekend red cases 12 week average]
```



```dax
Share of weekend red cases (0 if blank) = coalesce([Share of weekend red cases], 0)
```
...
```dax
_title Share of ""red"" cases with IC work greater or equal = "Share of ""red"" cases with non-project related work " & SELECTEDVALUE('IC Work more than selected hours'[Hours]) & "h or more"
```



```dax
_title Share of ""red"" cases with IC weekend work greater or equal = "Share of weekend work cases with non-project related work " & SELECTEDVALUE('IC weekend Work more than selected hours'[Hours]) & "h or more"
```



```dax
_title_flexibility = "Flexibility analysis - working until " & SELECTEDVALUE('Flexibility End Selected'[endInterval]) & " or later but 12h or less"
```
...
```dax
# End Interval answers = 
var _emp_ids = calculatetable(Values( 'Values by Staffing'[participant_email]))
return calculate( COUNTROWS(Values(DailyTimeRecording[ID])), TREATAS(_emp_ids, DailyTimeRecording[Mail]))
```



```dax
# End Interval total answers = calculate([# End Interval answers], REMOVEFILTERS(EndIntervals))
```



```dax
share of end time 4 week average = 
var _weeks = calculatetable(values('Calendar'[ShiftedWeekOffest]), all('Calendar'), 'Calendar'[ShiftedWeekOffest] <= 0 && 'Calendar'[ShiftedWeekOffest] > -4)  
return AVERAGEX( _weeks,   [share of end time])
```



```dax
share of end time = DIVIDE([# End Interval answers], [# End Interval total answers], Blank())
```



```dax
share of end time 8 week average = 
var _weeks = calculatetable(values('Calendar'[ShiftedWeekOffest]), all('Calendar'), 'Calendar'[ShiftedWeekOffest] <= 0 && 'Calendar'[ShiftedWeekOffest] > -8)  
return AVERAGEX( _weeks,   [share of end time])
```
...
```dax
share of end time 12 week average = 
var _weeks = calculatetable(values('Calendar'[ShiftedWeekOffest]), all('Calendar'), 'Calendar'[ShiftedWeekOffest] <= 0 && 'Calendar'[ShiftedWeekOffest] > -12)  
return AVERAGEX( _weeks,   [share of end time])
```
...
```dax
share of endtime 4 vs 8 = 
[share of end time 4 week average] - [share of end time 8 week average]
```



```dax
share of endtime 4 vs 12 = 
[share of end time 4 week average] - [share of end time 12 week average]
```
...
var _emp_ids = calculatetable(Values( 'Values by Staffing'[participant_email]))
var _less12hKeys = CALCULATEtable(SUMMARIZE('Q3 DidYouWorkLessThan12Hours', 'Q3 DidYouWorkLessThan12Hours'[Mail], 'Q3 DidYouWorkLessThan12Hours'[Reference Date]),  TREATAS(_emp_ids, DailyTimeRecording[Mail]), 'Q3 DidYouWorkLessThan12Hours'[Q3 Value] = 1)


return calculate(Countrows(Values(DailyTimeRecording[ID])), TREATAS(_less12hKeys, DailyTimeRecording[Mail], DailyTimeRecording[Reference Date]), DailyTimeRecording[End Interval] = "10 PM and later" ||  DailyTimeRecording[End Interval] = "12 AM and later")
```



```dax
share of end time (Until 10 pm) = CALCULATE([share of end time], EndIntervals[End Interval Flexibility] = "Until 10 pm")
```



```dax
share of end time (Until 12 am) = CALCULATE([share of end time], EndIntervals[End Interval Flexibility] = "Until 12 am")
```



```dax
share of end time (After 12 am) = CALCULATE([share of end time], EndIntervals[End Interval Flexibility] = "After 12 am")
```
...
```dax
FlexArrow 8 vs 4 = 
var _interval = SELECTEDVALUE(EndIntervals[End Interval Flexibility])

var _vs = [share of endtime 4 vs 8]
var _add = switch(true()
    , _vs < 0
    , -1, _vs > 0, 1
    , 0)

return switch(_interval
    , "Until 10 pm", 100 + _add
    , "Until 12 am", 200 + _add
    , "After 12 am", 300 + _add
)
```
...
var _vs = [share of endtime 4 vs 12]
var _add = switch(true()
    , _vs < 0
    , -1, _vs > 0, 1
    , 0)
...
```dax
# mentee = Countrows(values( 'Values by Staffing'[emp_id]))
```
...
```dax
count 12h red employees = calculate([# distinct employees], 'Q3 12h States'[State] = "Mostly no")
```



```dax
Share of 12h red cases = 

var _weeks = calculatetable(values('Calendar'[ShiftedWeekOffest]))
return AVERAGEX( _weeks,  
    var _red = coalesce( [count 12h red employees],0)
    var _part = [CountParticipant]
     return divide (_red, _part, blank())
)
```
...
```dax
Share of red cases 4 week average = 
var _weeks = calculatetable(values('Calendar'[ShiftedWeekOffest]), all('Calendar'), 'Calendar'[ShiftedWeekOffest] <= 0 && 'Calendar'[ShiftedWeekOffest] > -4)  
return AVERAGEX( _weeks,   [Share of 12h red cases])
...
```dax
Share of red cases 8 week average = 
var _weeks = calculatetable(values('Calendar'[ShiftedWeekOffest]), all('Calendar'), 'Calendar'[ShiftedWeekOffest] <= 0 && 'Calendar'[ShiftedWeekOffest] > -8)  
return AVERAGEX( _weeks,   [Share of 12h red cases])
```
...
```dax
Share of red cases 12 week average = 
var _weeks = calculatetable(values('Calendar'[ShiftedWeekOffest]), all('Calendar'), 'Calendar'[ShiftedWeekOffest] <= 0 && 'Calendar'[ShiftedWeekOffest] > -12)  
return AVERAGEX( _weeks,   [Share of 12h red cases])
```
...
```dax
Share of red cases 4 vs 8 = 
[Share of red cases 4 week average] - [Share of red cases 8 week average]
```



```dax
Share of red cases 4 vs 12 = 
[Share of red cases 4 week average] - [Share of red cases 12 week average]
```



```dax
count 12h red employees (multiple projects) = 
 var _sum =
  Calculatetable( SUMMARIZE( 'Values by Staffing',  'Values by Staffing'[emp_id],  "_dProjectCount", DISTINCTCOUNT('Values by Staffing'[ProjectNumber]))
  ,REMOVEFILTERS(Projetcs[Project] ))

var _emp_with_multiple_projects = Filter(_sum
  , [_dProjectCount] > 1)
  /*CALCULATETABLE(values('Values by Staffing'[emp_id]),
    Filter('Values by Staffing', 
        var _emp_id =  'Values by Staffing'[emp_id]
        return 'Values by Staffing'[emp_id] = _emp_id &&  countrows(values('Values by Staffing'[ProjectNumber]))> 1
        )
) */
 return //countrows(_emp_with_multiple_projects) 
  calculate([count 12h red employees], _emp_with_multiple_projects )
...
```dax
share 12h red (multiple projects) = 
var _weeks = calculatetable(values('Calendar'[ShiftedWeekOffest]))
return 
coalesce(AVERAGEX( _weeks,  
    divide(coalesce([count 12h red employees (multiple projects)], 0), [count 12h red employees], Blank())
), 0)
...
```dax
count 12h red employees (ic hours) = 
var _param_hours = [More than IC hours]
var _emp_with_ore_ic_hours = calculatetable(values('Values by Staffing'[emp_id]), 'Values by Staffing'[Q6/Q7 Hours] >= _param_hours) 
return calculate([count 12h red employees], _emp_with_ore_ic_hours)  //countrows(_emp_with_ore_ic_hours)  
```



```dax
share 12h red (IC hours) = 
var _weeks = calculatetable(values('Calendar'[ShiftedWeekOffest]))
return 
coalesce(AVERAGEX( _weeks,  
    divide(coalesce( [count 12h red employees (ic hours)], 0), [count 12h red employees], Blank())
), 0)
```
...
```dax
Share of 12h red cases (0 if blank) = coalesce([Share of 12h red cases],0)
```
...
```dax
calc participation rate based on 5 days * #Participants and #NoAnswer = 
Divide (coalesce( [calc # participants 5 days] - [# no answer (participants)], 0), [calc # participants 5 days], Blank())
```
...
PR 4 week average = 
var _weeks = calculatetable(values('Calendar'[ShiftedWeekOffest]), all('Calendar'), 'Calendar'[ShiftedWeekOffest] <= 0 && 'Calendar'[ShiftedWeekOffest] > -4)  
return AVERAGEX( _weeks,   [PR based on Q3])
```
...
```dax
PR 4 vs 8 week = 
[PR 4 week average]  - [PR 8 week average]
```
...
```dax
PR 4 vs 12 week = 
[PR 4 week average]  - [PR 12 week average]
```
...
var _weeks = calculatetable(values('Calendar'[ShiftedWeekOffest]))
return AVERAGEX( _weeks,  
    var _answ = coalesce( calculate([# distinct employees], 'Q3 12h States'[State] <> "n/a"),0)
    var _part = calculate([# distinct employees])
    return divide (_answ, _part, blank())
)
```
...
```dax
PR based on Q3 (0 if blank) = Coalesce([PR based on Q3],0)
```
...
var _res =  calculate([Q7 Hours], All('Calendar'), 'Calendar'[ShiftedWeekOffest] = _shifted_week_offset,  'Q7 HoursSpentOutsideProjectWeekend'[Mail] = _emp_mail )
return if(_res = 0, Blank(), _res)
```
...
```dax
PR (IDs) = divide( 5 -  [# no answer (participants)], 5,  blank())  
```
...
```dax
NoAnswerCount = 
var _off = LOOKUPVALUE('Calendar'[ShiftedWeekOffest], 'Calendar'[Date], 'Values by Staffing'[Reference Date])
var _mail = 'Values by Staffing'[participant_email]

return LOOKUPVALUE('no answer per emp and week'[NoAnswerCount], 'no answer per emp and week'[Mail], _mail, 'no answer per emp and week'[ShiftedWeekOffset], _off)
...
```dax
Participation Rate = DIVIDE(5 - 'Values by Staffing'[NoAnswerCount], 5) 
```
...
```dax
# no answer (participants) = 
var _part = calculatetable(values(Participants[participant_email]))
return calculate(COUNTROWS(values(SP_NoAnswer[ID])), TREATAS(_part, SP_NoAnswer[Mail]))
```
...
```dax
ShiftedWeekOffset = LOOKUPVALUE('Calendar'[ShiftedWeekOffest], 'Calendar'[Date],  SP_NoAnswer[Reference Date]) 
```
...
```dax
Filter(
  SUMMARIZECOLUMNS('Values by Staffing'[Reference Date],  'Values by Staffing'[emp_id], "_dProjectCount", DISTINCTCOUNT('Values by Staffing'[ProjectNumber]))
  , [_dProjectCount] > 1)
```
...
```dax
More than IC hours = min('IC Work more than selected hours'[Hours])
```
...
```dax
More than IC weekend hours = min('IC weekend Work more than selected hours'[Hours])
```


## Table: no answer per emp and week


```dax
SUMMARIZECOLUMNS(SP_NoAnswer[ShiftedWeekOffset], SP_NoAnswer[Mail], "NoAnswerCount", COUNT(SP_NoAnswer[ID]))
```
...
```dax
Average participation rate for emp and n. o. weeks = 
var _weekCount = SELECTEDVALUE(ParticipationExport_Weeks[weeks])
var _weeks = calculatetable(values('Calendar'[ShiftedWeekOffest]), all('Calendar'), 'Calendar'[ShiftedWeekOffest] <= 0 && 'Calendar'[ShiftedWeekOffest] > -1 * _weekCount) 
var _rate = SELECTEDVALUE(ParticipationExport_Rate[Rate])
var _emps = CALCULATETABLE(values('Values by Staffing'[emp_id]), _weeks)

return 
calculate(AVERAGEX(_emps, calculate(AVERAGE('Values by Staffing'[Participation Rate]))), _weeks)
...
```dax
IsVisible for selected Rate = if([Average participation rate for emp and n. o. weeks] < SELECTEDVALUE(ParticipationExport_Rate[Rate]), 1, 0)
```
...
```dax
Selected flexibility end = SELECTEDVALUE('Flexibility End Selected'[until])
```
...
# Flexibility answers = 
var _emp_ids = calculatetable(Values( 'Values by Staffing'[participant_email]))
var _until = [Selected flexibility end]

return  calculate( COUNTROWS(Values(DailyTimeRecording[ID])), TREATAS(_emp_ids, DailyTimeRecording[Mail]),  DailyTimeRecording[end hour] >= _until || DailyTimeRecording[Ends on next day] > 0)
```



```dax
# Flexibility answers <12h = 
var _emp_ids = calculatetable(Values( 'Values by Staffing'[participant_email]))
var _until = [Selected flexibility end]

return  calculate( COUNTROWS(Values(DailyTimeRecording[ID])), TREATAS(_emp_ids, DailyTimeRecording[Mail]),  DailyTimeRecording[is<12h] && (DailyTimeRecording[end hour] >= _until || DailyTimeRecording[Ends on next day] > 0))
```



```dax
share of flexibility = DIVIDE(coalesce( [# Flexibility answers <12h], 0), [# Flexibility answers]  )
```



```dax
share of flexibility 4 week average = 
var _weeks = calculatetable(values('Calendar'[ShiftedWeekOffest]), all('Calendar'), 'Calendar'[ShiftedWeekOffest] <= 0 && 'Calendar'[ShiftedWeekOffest] > -4)  
return  Averagex( Filter( ADDCOLUMNS(_weeks, "_count",  [# Flexibility answers],  "_share", [share of flexibility]), [_count]>0), [_share] )
//MAXx( Filter( ADDCOLUMNS(_weeks, "_count",  [# Flexibility answers],  "_share", [share of flexibility]), [_count]>0), 'Calendar'[ShiftedWeekOffest] )
```



```dax
share of flexibility 8 week average = 
var _weeks = calculatetable(values('Calendar'[ShiftedWeekOffest]), all('Calendar'), 'Calendar'[ShiftedWeekOffest] <= 0 && 'Calendar'[ShiftedWeekOffest] > -8)  
return AVERAGEX( _weeks,   [share of flexibility])
```



```dax
share of flexibility 12 week average = 
var _weeks = calculatetable(values('Calendar'[ShiftedWeekOffest]), all('Calendar'), 'Calendar'[ShiftedWeekOffest] <= 0 && 'Calendar'[ShiftedWeekOffest] > -12)  
return AVERAGEX( _weeks,   [share of flexibility])
```
...
```dax
share of flexibility 4 vs 8 = 
[share of flexibility 4 week average] - [share of flexibility 8 week average]
```



```dax
share of flexibility 4 vs 12 = 
[share of flexibility 4 week average] - [share of flexibility 12 week average]
```
