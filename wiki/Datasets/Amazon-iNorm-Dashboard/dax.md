...
```dax
Brand Type & Mix = CONCATENATE('wipes_dim Wide'[Clorox Brand Value],CONCATENATE("-",""))
```
...
SWITCH (
    ...
    [BU]
        ...
        "Facial Skin Care",
        "Beauty Brushes, Accessories & Tools",
        "Eye Care & Treatments"
    }
        && 'wipes_dim Wide'[Clorox Brand Value] IN{ "Fresh Step", "Burt's Bees"}, "CLX",
    [BU]
        IN {
        ...
        IN { "Hidden Valley", "Brita", "Natural Vitality", "Soy Vay", "Renew Life" }, "CLX",
    [BU]
        IN { "Food Storage" }
        && 'wipes_dim Wide'[Clorox Brand Value] = "Glad", "CLX",
    [BU]
        IN { "Grilling & Outdoor Fuels" }
        && 'wipes_dim Wide'[Clorox Brand Value] = "Kingsford", "CLX",
    'wipes_dim Wide'[Clorox Sub Category Value]
        IN {
        "All-Purpose Cleaners",
        "Bathroom Cleaners",
        "Household Disinfectant Wipes",
        "Household Drain Openers",
        "Household Kitchen Cleaners",
        "Liquid Toilet Cleaners",
        "Toilet Cleaning Systems",
        "Toilet Cleaning Tablets",
        "Cleaning Cloths",
        "Household Scouring Pads & Sticks"
    }
        && 'wipes_dim Wide'[Clorox Brand Value] IN { "Pine-Sol", "Clorox" }, "CLX",
    'wipes_dim Wide'[Clorox Sub Category Value]
        IN {
        "Household Floor Cleaners",
        "Household Glass Cleaners",
        "Other Disinfecting Wipes",
        "Cleaning Brushes",
        "Household Sponges"
    }
        && 'wipes_dim Wide'[Clorox Brand Value] IN { "Pine-Sol" }, "CLX",
    'wipes_dim Wide'[Clorox Sub Category Value]
        IN { "Compost Bags", "Trash Bags", "Trash Compactors & Compactor Bags" }
        && 'wipes_dim Wide'[Clorox Brand Value] = "Glad", "CLX",
...
```dax
Avg of $ / unit = CALCULATE(AVERAGE('wipes_val Wide'[$ / oz_All SKUs]),FILTER('wipes_val Wide','wipes_val Wide'[Year]= [selected_year]))
```
...
var en = IF(HASONEVALUE('wipes_dim Wide'[Product Key]),[Type Mix allocated 1],SUMX(SUMMARIZE('wipes_dim Wide','wipes_dim Wide'[Product]),[Type Mix allocated 1]))
...
VAR en = IF(HASONEVALUE('wipes_dim Wide'[Clorox Size Value]),[Size Mix allocated 1],SUMX(SUMMARIZE('wipes_dim Wide','wipes_dim Wide'[Clorox Size Value],'wipes_dim Wide'[Clorox Sub Category Value],'wipes_dim Wide'[Product]),[Size Mix allocated 1]))
...
SUMX(SUMMARIZE('wipes_val Wide','wipes_dim Wide'[Clorox Brand Value],'wipes_val Wide'[Product Key],'wipes_dim Wide'[Product],'wipes_dim Wide'[Clorox Sub Category Value]),[Mix Calculation])
)
...
```dax
PP_Total Units = CALCULATE(SUM('wipes_val Wide'[Units]),FILTER('wipes_val Wide','wipes_val Wide'[Year]=[selected_year]))
```
...
```dax
APP_Avg of $/unit = CALCULATE(AVERAGE('wipes_val Wide'[$ / oz_All SKUs]),FILTER('wipes_val Wide','wipes_val Wide'[Year]=[selected_year]))
```
...
% Volume = 
var dollar = CALCULATE(SUM('wipes_val Wide'[Volume Sales]),FILTER('wipes_val Wide','wipes_val Wide'[Year]=[selected_year]))
var deno = CALCULATE([PSA_Total volume share_cy],ALL('wipes_dim Wide'[Clorox Size Range Value]),ALL('wipes_dim Wide'[Clorox Size Value], 'wipes_dim Wide'[Size Calc],'wipes_dim Wide'[Index size range]))
...
lc $/oz wavg num (brand) = 
 CALCULATE(SUMX(
        'wipes_dim Wide',Measure_Table2[lc$/oz] *Measure_Table2[lc Total Revenue]),ALL('wipes_dim Wide'[Clorox Size Range Value],'wipes_dim Wide'[Index size range]))
```
...
DIVIDE([lc $/oz wavg num (brand)]
    ,
        CALCULATE([lc Total Revenue],ALL('wipes_dim Wide'[Clorox Size Range Value],'wipes_dim Wide'[Index size range])),0)
```
...
```dax
Swot_X Axis Lable = "Impact of driver("&SELECTEDVALUE('Revenue Drivers'[Drivers])&") for " & IF (
    SELECTEDVALUE ( 'wipes_dim Wide'[Business_Unit] ) = "BRITA",
    "BRITA",
IF (
                        ...
                        "BURTS BEES",
    IF (
        SELECTEDVALUE ( 'wipes_dim Wide'[BU] ) in{ "Food Storage","Trash, Recycling & Compost"},
        "GLAD",
        IF (
                    SELECTEDVALUE ( 'wipes_dim Wide'[BU] ) = "Cat Litter & Housebreaking","Clorox",
                    IF (SELECTEDVALUE ( 'wipes_dim Wide'[BU] ) = "Condiments & Salad Dressings","Hidden Valley",
                      IF (SELECTEDVALUE ( 'wipes_dim Wide'[BU] ) = "Grilling & Outdoor Fuels","Kingsford",
                      IF (SELECTEDVALUE ( 'wipes_dim Wide'[Clorox Sub Category Value] ) IN{ "Household Floor Cleaners","Other Disinfecting Wipes","Cleaning Brushes","Household Glass Cleaners","Household Sponges"},"Pine-Sol",
                   IF (SELECTEDVALUE ( 'wipes_dim Wide'[BU] )="Minerals","Natural Vitality",
                   IF (SELECTEDVALUE ( 'wipes_dim Wide'[BU] )In { "Sauces & Gravies"},"Soy Vay",
                   IF (SELECTEDVALUE ( 'wipes_dim Wide'[BU] )="Supplements","NeoCell",
    ...
    IF (
        ...
        IF (
                    ...
                    IF (SELECTEDVALUE ( 'wipes_dim Wide'[BU] ) = "Condiments & Salad Dressings","Hidden Valley",
                      IF (SELECTEDVALUE ( 'wipes_dim Wide'[BU] ) = "Grilling & Outdoor Fuels","Kingsford",
                      IF (SELECTEDVALUE ( 'wipes_dim Wide'[Clorox Sub Category Value] ) IN{ "Household Floor Cleaners","Other Disinfecting Wipes","Cleaning Brushes","Household Glass Cleaners","Household Sponges"},"Pine-Sol",
                   IF (SELECTEDVALUE ( 'wipes_dim Wide'[Clorox Sub Category Value] )="Magnesium Supplements","Natural Vitality",
                   IF (SELECTEDVALUE ( 'wipes_dim Wide'[Clorox Sub Category Value] )In { "Barbecue Sauce","Other Sauces & Gravies","Sweet & Sour Sauce","Teriyaki Sauce"},"Soy Vay",
                   IF (SELECTEDVALUE ( 'wipes_dim Wide'[Clorox Sub Category Value] )="Other Dietary Supplements","NeoCell",
                SELECTEDVALUE ( 'wipes_dim Wide'[New Clorox Brand Value] )
        )
    )
))) ))))) & ")"
...
CALCULATE([Par_$_selected_year],ALL('wipes_dim Wide'[Product],'wipes_dim Wide'[Product Key]))
...
```dax
Axis Lable for Price & promo = "Price Analysis For " &SELECTEDVALUE('wipes_dim Wide'[Clorox Brand Value])
```
...
```dax
Par_$_Rank = RANKX(ALL('wipes_dim Wide'[Product],'wipes_dim Wide'[Product Key]),[Par_$_selected_year],,DESC)
```
...
```dax
Par_$_%chg_year = 
var py_= CALCULATE(SUM('wipes_val Wide'[$]),FILTER('wipes_val Wide','wipes_val Wide'[Year]=[py_year]))

return
DIVIDE(([Par_$_selected_year]-py_),py_,0)
```
