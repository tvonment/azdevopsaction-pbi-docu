...
let
    ...
    #"Merged Queries" = Table.NestedJoin(#"Umbenannte Spalten3", {"Sector", "Reference Year-Month Date"}, #"Umbenannte Spalten3", {"Sector", "Year-Month Date"}, "Umbenannte Spalten3", JoinKind.LeftOuter),
    ...
    #"Changed Type7" = Table.TransformColumnTypes(#"Added Custom5",{{"Reference Change Year-to-Year", type number}, {"Year", Int64.Type}})
in
    #"Changed Type7"
```
...
let
    ...
    #"Filtered Rows" = Table.SelectRows(#"Hinzugefügte benutzerdefinierte Spalte1", each ([#"Reference Change Year-to-Year"] <> null)),
    #"Sorted Rows" = Table.Sort(#"Filtered Rows",{{"Year-Month Date", Order.Ascending}}),
    #"Changed Type" = Table.TransformColumnTypes(#"Sorted Rows",{{"Reference CPI Value", type number}, {"Reference Change Year-to-Year", type number}, {"CPI Value", type number}, {"Year-Month Date", type date}})
in
    #"Changed Type"
```
...
let
   Source = Excel.Workbook(File.Contents("C:\Users\M712182\Roland Berger Holding GmbH\MoCI Inflation Study - General\07 Dashboards\MoCI Power BI MASTER data.xlsx"), null, true),
    ...
    #"Changed Type" = Table.TransformColumnTypes(#"Promoted Headers",{{"CPI Value", type number}, {"Index", type text}, {"Country", type text}, {"Sector", type text}, {"Year", Int64.Type}, {"Month", Int64.Type}, {"Year-Month String", type date}, {"Year-Month Date", type date}}),
...
let
    ...
    #"Filtered Rows" = Table.SelectRows(#"Changed Type1", each ([Sector] <> "Printing And Reproduction Of Recorded Media")),
    #"Changed Type2" = Table.TransformColumnTypes(#"Filtered Rows",{{"Year", Int64.Type}})
in
    #"Changed Type2"
```
...
let
    ...
    #"Merged Queries" = Table.NestedJoin(#"Umbenannte Spalten3", {"Sector", "Reference Year-Month Date"}, #"Umbenannte Spalten3", {"Sector", "Year-Month Date"}, "Removed Columns", JoinKind.LeftOuter),
    #"Expanded Removed Columns1" = Table.ExpandTableColumn(#"Merged Queries", "Removed Columns", {"CPI Value"}, {"Removed Columns.CPI Value"}),
    #"Added Custom" = Table.AddColumn(#"Expanded Removed Columns1", "Reference Change Year-to-Year", each ([CPI Value]-[Removed Columns.CPI Value])/[Removed Columns.CPI Value]),
...
let
    ...
    #"Changed Type" = Table.TransformColumnTypes(#"Promoted Headers",{{"Index", type text}, {"Country", type text}, {"Sector", type text}, {"INDICATOR", type text}, {"source", type text}, {"Year", Int64.Type}, {"Month", Int64.Type}, {"CPI sector weight", type number}, {"Year-Month String", type date}, {"Year-Month Date", type date}}),
    #"Added Custom" = Table.AddColumn(#"Changed Type", "CPI sector weight TEMP", each [CPI sector weight]/100),
    #"Removed Columns" = Table.RemoveColumns(#"Added Custom",{"CPI sector weight"}),
    #"Renamed Columns" = Table.RenameColumns(#"Removed Columns",{{"CPI sector weight TEMP", "CPI sector weight"}}),
    #"Changed Type1" = Table.TransformColumnTypes(#"Renamed Columns",{{"CPI sector weight", type number}})
in
    #"Changed Type1"
```
