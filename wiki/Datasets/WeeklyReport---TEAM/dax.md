...
RLS Current User Mail = 
    if(
       //USERPRINCIPALNAME() = "claudia.fischer-mayer@rolandberger.com", "mona.benzekri@rolandberger.com"
...
```dax
Diff Avg Cw to LW = if(ISNUMBER( [Avg Answer in Last Week]) &&  ISNUMBER([Avg Current Or Last Week (by visual filter)]), DIVIDE([Avg Current Or Last Week (by visual filter)] - [Avg Answer in Last Week], [Avg Answer in Last Week])
,"")
```
...
var responsiblePm = LOOKUPVALUE(Teams[ResponsiblePMEmail],Teams[ProjectSurveyId], selectedProject, Teams[EmployeeMail], USERPRINCIPALNAME()) 
...
```dax
Responsible PM = [TEAM Responsible PM]
/*var selectedProject = SELECTEDVALUE(DimProjectSurvey[ProjectSurveyID])
...
var responsiblePm = LOOKUPVALUE(Teams[ResponsiblePMEmail],Teams[ProjectSurveyId], selectedProject, Teams[EmployeeMail], USERPRINCIPALNAME()) 
//var responsiblePm = FactSurveyQ[Responsible PM]
return if(ISBLANK(responsiblePm), blank(), responsiblePm) //calculate([Avg Min 3 values],Filter(DimParticipants, DimParticipants[ResponsiblePMEmail] = responsiblePm))  
```
