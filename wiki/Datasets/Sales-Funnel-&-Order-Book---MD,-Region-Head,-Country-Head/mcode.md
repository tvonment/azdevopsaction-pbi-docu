...
let
    ...
    #"Filtered Rows" = Table.SelectRows(pub_v_ll_company_to_region, each ([company_id] <> "77") and ([org_unit_id] <> null)),
...
```m
let
    Source = #"sec imp_fc_order_income_accumulated_weekly_source",
    #"Merged Queries" = Table.NestedJoin(Source, {"project_number", "industry", "function", "report_week_last_id"}, #"sec imp_fc_order_income_accumulated_weekly_source", {"project_number", "industry", "function", "report_week_current_id"}, "sec imp_fc_order_income_accumulated_weekly_source", JoinKind.LeftOuter),
    #"Expanded sec imp_fc_order_income_accumulated_weekly_source" = Table.ExpandTableColumn(#"Merged Queries", "sec imp_fc_order_income_accumulated_weekly_source", {"order_income_total"}, {"order_income_last_week"}),
    #"Replaced Value" = Table.ReplaceValue(#"Expanded sec imp_fc_order_income_accumulated_weekly_source",null,0,Replacer.ReplaceValue,{"order_income_last_week"}),
    #"Added Custom" = Table.AddColumn(#"Replaced Value", "order_income_current_week", each [order_income_total] - [order_income_last_week], type number),
    #"Gefilterte Zeilen" = Table.SelectRows(#"Added Custom", each [report_week_current_id] >= 39)
in
    #"Gefilterte Zeilen"
```
