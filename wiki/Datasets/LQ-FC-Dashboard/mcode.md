...
let
    ...
    #"Filtered Plan Variant" = Table.SelectRows(dbo_V_DW_LIP_DATA_SUM, each [PLAN_VARIANT_ID] > 112),
    ...
    #"Removed Columns1" = Table.RemoveColumns(#"Changed Type2",{"YearWeekOfYear(LeadingZero)", "Week of Year( Leading Zero) "})
in
    #"Removed Columns1"
```
...
let
    ...
    #"Inserted Day Name" = Table.AddColumn(#"Sorted Rows", "Day Name", each Date.DayOfWeekName([DATUM]), type text),
    #"Added Custom" = Table.AddColumn(#"Inserted Day Name", "Custom", each Date.StartOfWeek(DateTime.Date( DateTime.LocalNow() ))),
    #"Renamed Columns" = Table.RenameColumns(#"Added Custom",{{"Custom", "Current week Monday"}}),
    #"Added Conditional Column" = Table.AddColumn(#"Renamed Columns", "Till Monday dates filter", each if [DATUM] <= [Current week Monday] then 1 else 0),
    #"Filtered Rows1" = Table.SelectRows(#"Added Conditional Column", each [Till Monday dates filter] = 1),
    #"Changed Type1" = Table.TransformColumnTypes(#"Filtered Rows1",{{"Current week Monday", type date}})
in
    #"Changed Type1"
```
...
let
    ...
    #"Filtered Rows2" = Table.SelectRows(#"Renamed Columns", each ([UNIQUE_IDENTIFIER] <> "DH Diagnostics, LLC // CP10077: Multiple Myeloma Dx Market Study / SE-150-202110512.0000") and ([CUSTOMER_INVOICE_ID] <> "88-226-2022"))
in
    #"Filtered Rows2"
```
...
let
    ...
    #"Filtered Rows" = Table.SelectRows(dbo_V_DW_LIP_PLAN, each ([PLAN_START_PERIODE] = "2022" or [PLAN_START_PERIODE] = "2023"))
in
    #"Filtered Rows"
```
...
let
    ...
    #"Last day of FC" = Table.AddColumn(Source_date, "Custom.1", each List.Dates([PLAN_START], 104, #duration(1, 0, 0, 0))),
...
let
    ...
    #"20221005_to avoid CW53" = Table.SelectRows(#"Removed Duplicates", each ([Days] <> #date(2023, 1, 1))),
    #"Inserted Year" = Table.AddColumn(#"20221005_to avoid CW53", "Year", each Date.Year([Days]), Int64.Type),
    ...
    #"Removed Columns" = Table.RemoveColumns(#"Changed Type1",{"Week of Year", "Year", "Y_CW_long"})
in
    #"Removed Columns"
```
...
Source,
    #"Changed Type" = Table.TransformColumnTypes(#"Converted to Table",{{"Date Last Refreshed", type datetimezone}}),
    #"Duplicated Column" = Table.DuplicateColumn(#"Changed Type", "Date Last Refreshed", "Date Last Refreshed - Copy"),
    #"Duplicated Column1" = Table.DuplicateColumn(#"Duplicated Column", "Date Last Refreshed", "Date Last Refreshed - Copy.1"),
    #"Reordered Columns" = Table.ReorderColumns(#"Duplicated Column1",{"Date Last Refreshed", "Date Last Refreshed - Copy.1", "Date Last Refreshed - Copy"}),
    #"Extracted Last Characters" = Table.TransformColumns(#"Reordered Columns", {{"Date Last Refreshed - Copy", each Text.End(Text.From(_, "de-DE"), 4), type text}}),
    #"Changed Type1" = Table.TransformColumnTypes(#"Extracted Last Characters",{{"Date Last Refreshed", type datetime}, {"Date Last Refreshed - Copy", type datetime}}),
    #"Renamed Columns" = Table.RenameColumns(#"Changed Type1",{{"Date Last Refreshed - Copy", "UTC * time"}, {"Date Last Refreshed - Copy.1", "Time"}}),
    #"Duplicated Column2" = Table.DuplicateColumn(#"Renamed Columns", "Date Last Refreshed", "Date Last Refreshed - Copy"),
    #"Reordered Columns1" = Table.ReorderColumns(#"Duplicated Column2",{"Date Last Refreshed", "Date Last Refreshed - Copy", "Time", "UTC * time"}),
    #"Extracted First Characters" = Table.TransformColumns(#"Reordered Columns1", {{"Date Last Refreshed - Copy", each Text.Start(Text.From(_, "de-DE"), 10), type text}}),
    #"Renamed Columns1" = Table.RenameColumns(#"Extracted First Characters",{{"Date Last Refreshed - Copy", "Date"}}),
    #"Changed Type2" = Table.TransformColumnTypes(#"Renamed Columns1",{{"Time", type time}, {"UTC * time", type time}}),
    #"Extracted Hour" = Table.TransformColumns(#"Changed Type2",{{"UTC * time", Time.Hour, Int64.Type}}),
    #"Added Custom" = Table.AddColumn(#"Extracted Hour", "UTC added time", each [#"UTC * time"]/24),
    #"Changed Type3" = Table.TransformColumnTypes(#"Added Custom",{{"UTC added time", type number}}),
    #"Rounded Off" = Table.TransformColumns(#"Changed Type3",{{"UTC added time", each Number.Round(_, 3), type number}})
in
    #"Rounded Off"
```
...
```m
if(lookupvalue([HAS_ADMIN_RIGHTS], [EMAIL], Userprincipalname()) <> "Y", [EMAIL] = Userprincipalname(), [HAS_ADMIN_RIGHTS] <> " ")
```
