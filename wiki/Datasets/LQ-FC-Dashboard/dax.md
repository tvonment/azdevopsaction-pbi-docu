...
|Dataset|[LQ FC Dashboard](./../LQ-FC-Dashboard.md)|
| :--- | :--- |
...
```dax
CIB last monday_Cash out = [CIB as of Current week Monday]+ '1_Measure_FC'[FC_Amount_RFC_Cash Out]
```
...
```dax
current week Monday_ID = if(ACTUAL_BALANCES[DATUM]= ACTUAL_BALANCES[Current week Monday], 1,0)
```
...
```dax
CIB as of Current week Monday = calculate(sum(ACTUAL_BALANCES[TOTAL_SUM_EUR]), ACTUAL_BALANCES[TOTAL_SUM_EUR] > 0, ACTUAL_BALANCES[current week Monday_ID]=1)
```
...
```dax
CIB 2023 = calculate(sum(ACTUAL_BALANCES[TOTAL_SUM_EUR]),ACTUAL_BALANCES[Year] = {2023}, ACTUAL_BALANCES[Day of Week] = {1}, ACTUAL_BALANCES[TOTAL_SUM_EUR] > 0)
```



```dax
Credit Line_2023 = CALCULATE(sum('RB_RoW Credit Lines'[Amount_EUR]),'RB_RoW Credit Lines'[Year] = {2023}, 'RB_RoW Credit Lines'[Day of Week] = {6})
```



```dax
Credit_Line_Usage 2023 = calculate(sum(ACTUAL_BALANCES[TOTAL_SUM_EUR]),ACTUAL_BALANCES[Year] = {2023}, ACTUAL_BALANCES[Day of Week] = {1}, ACTUAL_BALANCES[TOTAL_SUM_EUR] < 0)
```
...
## Table: RB_RoW Credit Lines

### Calculated Columns:


```dax
FX Rate = LOOKUPVALUE(ACTUAL_FX_RATES[RATES_VALUE], ACTUAL_FX_RATES[DATUM], 'RB_RoW Credit Lines'[Date], ACTUAL_FX_RATES[CURRENCY], 'RB_RoW Credit Lines'[Currency])
```



```dax
Amount_EUR = if('RB_RoW Credit Lines'[Date] < TODAY()+1, 'RB_RoW Credit Lines'[Amount] / 'RB_RoW Credit Lines'[FX Rate], BLANK())
```
...
## Table: ZZ_DateLastRefresh Adjusted Zone

### Calculated Columns:


```dax
Adjusted Zone Time = 'ZZ_DateLastRefresh Adjusted Zone'[Date Last Refreshed] + 'ZZ_DateLastRefresh Adjusted Zone'[UTC added time]
```
