...
## Table: hide_employees


```m
let
    Source = Table.FromRows(Json.Document(Binary.Decompress(Binary.FromText("i45W8jUwMLIwtlSK1QGxzUyMDIzhbBNLAwjb3MDA3MQUxjYyNoOLmxqYmsDYZuZm5lC2oYEZSG8sAA==", BinaryEncoding.Base64), Compression.Deflate)), let _t = ((type nullable text) meta [Serialized.Text = true]) in type table [emp_id = _t]),
    #"Changed Type" = Table.TransformColumnTypes(Source,{{"emp_id", type text}}),
    #"Cleaned Text" = Table.TransformColumns(#"Changed Type",{{"emp_id", Text.Clean, type text}})
in
    #"Cleaned Text"
```
...
let
    ...
    rep_v_ll_CC = Source{[Schema="rep",Item="v_ll_CC"]}[Data]
in
    rep_v_ll_CC
```
