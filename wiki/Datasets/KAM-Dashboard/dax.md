...
```dax
submitdate (bins) = IF(
	ISBLANK('salesorders'[submitdate]),
	BLANK(),
	DATE(
		YEAR('salesorders'[submitdate]),
		1 + (MONTH('salesorders'[submitdate]) - 1),
		1
	)
)
```
...
## Table: calendar_salesorder_submitdate
...
```dax
Date (bins) = IF(
	ISBLANK('calendar_salesorder_submitdate'[Date]),
	BLANK(),
	DATE(
		YEAR('calendar_salesorder_submitdate'[Date]),
		1 + (MONTH('calendar_salesorder_submitdate'[Date]) - 1),
		1
	)
)
```
...
## Table: calendar_opportunity_exporderdate
...
```dax
Date (bins) = IF(
	ISBLANK('calendar_opportunity_exporderdate'[Date]),
	BLANK(),
	DATE(
		YEAR('calendar_opportunity_exporderdate'[Date]),
		1 + (MONTH('calendar_opportunity_exporderdate'[Date]) - 1),
		1
	)
)
```
