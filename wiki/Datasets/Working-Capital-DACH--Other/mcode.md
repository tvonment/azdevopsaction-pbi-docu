...
|Dataset|[Working Capital DACH- Other](./../Working-Capital-DACH--Other.md)|
| :--- | :--- |
...
let
    ...
    #"Filtered Rows1" = Table.SelectRows(#"Added Custom", each ([COMPANY] = "Austria" or [COMPANY] = "Germany" or [COMPANY] = "Switzerland")),
...
let
    ...
    #"Expanded Controller" = Table.ExpandTableColumn(#"Merged Queries", "Controller", {"F&C Reponsible"}, {"Controller.F&C Reponsible"}),
    #"Appended Query" = Table.Combine({#"Expanded Controller", SAP_RECEIABLES_DOWNPAYMENTS_TEST}),
    #"Added Conditional Column" = Table.AddColumn(#"Appended Query", "Downpayment ID", each if [#"TYPE(DP/AR)"] = "Downpayments" then "Y" else "N"),
    #"Filtered Rows" = Table.SelectRows(#"Added Conditional Column", each ([RECEIVABLE_COMPANY] = "Austria" or [RECEIVABLE_COMPANY] = "Germany" or [RECEIVABLE_COMPANY] = "Switzerland")),
    #"Added Conditional Column1" = Table.AddColumn(#"Filtered Rows", "Project ID with DP", each if [Downpayment ID] = "Y" then [SAP_CUSTOMER_DOWN_PAYMENTS_Open.PROJECT_ID] else [PROJECT_ID]),
    #"Changed Type" = Table.TransformColumnTypes(#"Added Conditional Column1",{{"NET_DUE_DATE_ADJUSTED", type date}, {"NET_DUE_DATE", type date}, {"INVOICE_DATE", type date}})
in
    #"Changed Type"
```
...
## Table: SAP_RECEIABLES_DOWNPAYMENTS_TEST
...
let
    Source = Sql.Databases("muc-mssql-1a.rolandberger.net"),
    WorkingCapitalMgmt = Source{[Name="WorkingCapitalMgmt"]}[Data],
    dbo_SAP_RECEIABLES_DOWNPAYMENTS_TEST = WorkingCapitalMgmt{[Schema="dbo",Item="SAP_RECEIABLES_DOWNPAYMENTS_TEST"]}[Data],
    #"Filtered Rows" = Table.SelectRows(dbo_SAP_RECEIABLES_DOWNPAYMENTS_TEST, each true),
    #"Filtered Rows1" = Table.SelectRows(#"Filtered Rows", each [OPEN_AMOUNT] > 1),
    #"Removed Columns" = Table.RemoveColumns(#"Filtered Rows1",{"PROJECT_ID"}),
    #"Merged Queries" = Table.NestedJoin(#"Removed Columns", {"CUSTOMER_INVOICE_ID"}, SAP_CUSTOMER_DOWN_PAYMENTS_Open, {"INVOICE_ID"}, "SAP_CUSTOMER_DOWN_PAYMENTS_Open", JoinKind.LeftOuter),
    #"Expanded SAP_CUSTOMER_DOWN_PAYMENTS_Open" = Table.ExpandTableColumn(#"Merged Queries", "SAP_CUSTOMER_DOWN_PAYMENTS_Open", {"CANCELLATION_INVOICE_INDICATOR_ID", "PROJECT_ID"}, {"SAP_CUSTOMER_DOWN_PAYMENTS_Open.CANCELLATION_INVOICE_INDICATOR_ID", "SAP_CUSTOMER_DOWN_PAYMENTS_Open.PROJECT_ID"}),
    #"Filtered Rows2" = Table.SelectRows(#"Expanded SAP_CUSTOMER_DOWN_PAYMENTS_Open", each ([SAP_CUSTOMER_DOWN_PAYMENTS_Open.CANCELLATION_INVOICE_INDICATOR_ID] = "False")),
    #"Renamed Columns" = Table.RenameColumns(#"Filtered Rows2",{{"TYPE", "TYPE(DP/AR)"}}),
    #"Inserted Merged Column" = Table.AddColumn(#"Renamed Columns", "Unique_Project&Invoice_ID", each Text.Combine({[SAP_CUSTOMER_DOWN_PAYMENTS_Open.PROJECT_ID], [CUSTOMER_INVOICE_ID]}, "_"), type text),
    #"Inserted Merged Column1" = Table.AddColumn(#"Inserted Merged Column", "Unique_3", each Text.Combine({[SAP_CUSTOMER_DOWN_PAYMENTS_Open.PROJECT_ID], [CUSTOMER_INVOICE_ID], [CUSTOMER]}, "_"), type text)
in
...
## Table: SAP_CUSTOMER_DOWN_PAYMENTS_Open & Paid for WIP (2)
