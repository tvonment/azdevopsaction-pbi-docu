...
```dax
Fee Budget TC (part) = if(dim_project[IsMaster]=TRUE()
    , CALCULATE([Fee Budget TC],ALLEXCEPT(dim_project, dim_project[Main Project Number]), dim_project[IsMaster]) - CALCULATE([Fee Budget TC],ALLEXCEPT(dim_project, dim_project[Main Project Number]), dim_project[IsMaster]=false)
    ,[Fee Budget TC]) 
```
...
```dax
IE Budget TC (part) = if(dim_project[IsMaster]=TRUE()
    , CALCULATE([IE Budget TC],ALLEXCEPT(dim_project, dim_project[Main Project Number]), dim_project[IsMaster]) - CALCULATE([IE Budget TC],ALLEXCEPT(dim_project, dim_project[Main Project Number]), dim_project[IsMaster]=false)
    ,[IE Budget TC]) 
```
...
```dax
Total Budget TC (part) = dim_project[Fee Budget TC (part)] + dim_project[IE Budget TC (part)] 
```



```dax
Invoiced Fee TC (part) = if(dim_project[IsMaster]=TRUE()
    ,Blank()
    ,[Invoiced Fee TC]) 
```
...
```dax
Invoiced IE TC (part) = if(dim_project[IsMaster]=TRUE()
    ,Blank()
    ,[Invoiced IE TC]) 
```
...
```dax
Total Invoiced Revenue TC (part) = dim_project[Invoiced Fee TC (part)] + dim_project[Invoiced IE TC (part)]
```
...
```dax
Net Value TC = Sum('rep v_fc_psr_invoice'[CLEARED_AMOUNT_TC]) 
...
```dax
Indicative Fee Completed TC = CALCULATE([Invoiced Fee TC],dim_project[project_status] = "Closed")
```
...
var calc=
calculate(
    SUM(rep_v_fc_byd_customer_project_overview[net_value_ordered] )
  , Filter(rep_v_fc_byd_customer_project_overview, [Budget_Type]= "Fee Budget")
    )
...
IE Budget TC = 
 calculate(
    SUM(rep_v_fc_byd_customer_project_overview[net_value_ordered])
   , Filter(rep_v_fc_byd_customer_project_overview, [Budget_Type]= "IE Budget")
   )
```
...
```dax
Total Budget TC = [Fee Budget TC] + [IE Budget TC]
```
...
Invoiced Fee TC = 
calculate(
    SUM(rep_v_fc_byd_customer_project_overview[net_value_invoiced])
    , Filter(rep_v_fc_byd_customer_project_overview, [Budget_Type] = "FEE Budget" )
    )
```
...
```dax
Invoiced Revenue TC = SUM(rep_v_fc_byd_customer_project_overview[net_value_invoiced])
```



```dax
Fee internal costs @100% incl. Sub TC = if(HASONEVALUE(dim_project[Main Project Number])
    ,CALCULATE([Σ Fee Used TC], filter(All(dim_project), dim_project[Main Project Number] = max(dim_project[Main Project Number])))
    ,CALCULATE([Σ Fee Used TC], filter(All(dim_project), dim_project[Main Project Number] in Values(dim_project[Main Project Number])))
)
...
```dax
IE internal costs incl. Sub TC = 
 CALCULATE([Σ IE Used TC], filter(All(dim_project), dim_project[Main Project Number] in Values(dim_project[Main Project Number]) ), All(dim_employee))
```



```dax
Invoiced and Paid TC = sum('rep v_fc_psr_invoice'[CLEARED_AMOUNT_NET_TC])
```



```dax
Unpaid Invoices TC = [OPEN AMOUNT]
```



```dax
Indicative Fee Revenue TC = 
    if(
        min(dim_project[project_status]) = "Released"
            , [Fee Budget TC]* [Project Fee Completion]
            , [Fee Budget TC]
    )
```
...
```dax
Indicative IE Revenue (main) TC = if(
        min(dim_project[project_status]) = "Released"
            , [IE internal costs incl. Sub TC]
            , [IE Budget TC] 
    )
...
```dax
Total Indicative Revenue (main) TC = [Indicative Fee Revenue TC]+ [Indicative IE Revenue (main) TC]
```



```dax
Actual OD Fee incl. Sub TC = [Fee internal costs @100% incl. Sub TC] - [Indicative Fee Revenue TC]
```



```dax
Actual Overdraft IE incl. Sub TC = [IE internal costs incl. Sub TC] - [IE Budget TC]
```



```dax
Total Actual Overdraft incl. Sub TC = [Actual OD Fee incl. Sub TC] + [Actual Overdraft IE incl. Sub TC]  
```



```dax
Open WIP Fee TC = if(min(dim_project[project_status]) = "Closed",0 ,[Indicative Fee Revenue TC]-[Invoiced Fee TC])
```



```dax
Project Margin incl. Sub TC = if(and(HASONEVALUE(dim_project[project_status]),min(dim_project[project_status])="Closed"),[Invoiced Revenue TC] -([Fee internal costs @100% incl. Sub TC] + [IE internal costs incl. Sub TC]- [Fee internal costs @100% employee incl. Sub TC]*0.35), Blank())
```



```dax
Fee internal costs @100% employee incl. Sub TC = 
if(HASONEVALUE(dim_project[Main Project Number])
    ,CALCULATE([Σ Fee Used employee TC], filter(All(dim_project), dim_project[Main Project Number] = max(dim_project[Main Project Number] )))
    ,CALCULATE([Σ Fee Used employee TC], filter(All(dim_project), dim_project[Main Project Number] in Values(dim_project[Main Project Number]))))
```



```dax
Actual OD Fee % incl. Sub TC = DIVIDE([Actual OD Fee incl. Sub TC], [Indicative Fee Revenue TC])
```



```dax
Project Margin % incl. Sub TC = Divide ([Project Margin incl. Sub TC], [Invoiced Revenue TC])
```



```dax
Planned IE Ratio as of Fee TC = DIVIDE([IE Budget TC],[Fee Budget TC])
```



```dax
Actual IE Ratio as of Fee incl. Sub TC = DIVIDE([IE internal costs incl. Sub TC],[Fee Budget TC])
```



```dax
Internal (not I/C) costs @ 100% incl. Sub TC = [Fee internal costs @100% incl. Sub TC] + [IE internal costs incl. Sub TC]
```



```dax
Fee Indicative Remaining (main) TC = [Fee Budget TC] - [Indicative Fee Revenue TC]
```
...
```dax
Dynamic Table Value TC = 
Switch(min(DisplayProjectTableData[Field])
    ,"Fee Budget", [Fee Budget TC]
    , "IE Budget", [IE Budget TC]
    ,"Total Budget", [Total Budget TC]

    ,"Fee Invoiced", [Invoiced Fee TC]
    ,"IE Invoiced", [Invoiced IE TC]
    ,"Total Invoiced", [Invoiced Revenue TC]

    ,"Fee Used", [Fee internal costs @100% incl. Sub TC]
    
    ,"IE used", [IE internal costs incl. Sub TC]
    ,"Total Used", [Σ Total Used incl. Sub TC]

    ,"FEE Actual OD", [Actual OD Fee incl. Sub TC]
    ,"IE Actual OD", [Actual Overdraft IE incl. Sub TC]
    ,"Total Actual OD", [Total Actual Overdraft incl. Sub TC]

    ,"Total Invoice Paid", [Invoiced and Paid TC]
    ,"Total Invoice Unpaid", [Unpaid Invoices TC]

    ,"Open WIP Fee", [Open WIP Fee TC]

    ,"Fee Indicative Revenue", [Indicative Fee Revenue TC]
    ,"IE Indicative Revenue", [Indicative IE Revenue (main) TC]
    ,"Total Indicative Revenue",[Total Indicative Revenue (main) TC]
    ,"Project Margin", [Project Margin incl. Sub TC]
    ,"Fee Indicative Remaining", if(min(dim_project[project_status]) = "Closed" ,"-", [Fee Budget TC]  - [Indicative Fee Revenue TC])
...
Switch(min(DisplayProjectTableData[Field])
    ...
    ,"Time Completion", [Project Time Completion]
    ,"IE Completion", [Project IE Completion (main)]
    ,"IE Planned in % of Fee", [Planned IE Ratio as of Fee TC]
    ,"IE Actual in % of Fee", [Actual IE Ratio as of Fee incl. Sub TC]
    ,"Project Margin", [Project Margin % incl. Sub TC]
, Blank()) 
```
...
```dax
Σ Total Used TC = 
 [Σ Fee Used TC] + [Σ IE Used TC]
```



```dax
Σ Fee Used employee TC = calculate(Sum(v_fc_psr_byd_project_internal_incurred_costs[fee_internal_costs_100pct_tc]),Filter(dim_employee, dim_employee[emp_id] <> blank() && dim_employee[emp_id] <> "-999"))
```



```dax
Σ Fee Used TC = calculate(Sum(v_fc_psr_byd_project_internal_incurred_costs[fee_internal_costs_100pct_tc]))
```



```dax
Σ IE Used TC = Sum(v_fc_psr_byd_project_internal_incurred_costs[ie_internal_costs_tc])
```



```dax
Σ Total Used incl. Sub TC = 
[Fee internal costs @100% incl. Sub TC] + [IE internal costs incl. Sub TC]
```
