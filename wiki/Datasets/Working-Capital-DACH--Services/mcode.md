...
let
    ...
    #"Added Conditional Column2" = Table.AddColumn(#"Changed Type", "Open Amount Positive ID", each if [OPEN_WIP] > 1 then 1 else 0),
    #"Added Custom" = Table.AddColumn(#"Added Conditional Column2", "Overdue ID", each if [AGE_OF_WIP] > 30 and [OPEN_WIP] > 10 then 1 else 0),
    #"Filtered Rows2" = Table.SelectRows(#"Added Custom", each Text.StartsWith([PROJECT_ID], "cp")),
    #"Added Conditional Column" = Table.AddColumn(#"Filtered Rows2", "AGE_OF_WIP_for Traffic light", each if [OPEN_WIP] > 1 then [AGE_OF_WIP] else 0),
    #"Added Custom1" = Table.AddColumn(#"Added Conditional Column", "Overdue WIP Age", each if [AGE_OF_WIP] > 30 then [AGE_OF_WIP]-30 else 0),
    #"Changed Type1" = Table.TransformColumnTypes(#"Added Custom1",{{"Overdue WIP Age", type number}}),
    #"Added Custom2" = Table.AddColumn(#"Changed Type1", "Overdue Open WIP", each if [Overdue ID] = 1 then [OPEN_WIP] else 0)
in
    #"Added Custom2"
```
...
let
    ...
    #"Added Custom1" = Table.AddColumn(#"Changed Type", "Overdue ID", each if [OPEN_AMOUNT] > 10 and [DAYS_OVERDUE] > 0 then 1 else 0),
    #"Changed Type1" = Table.TransformColumnTypes(#"Added Custom1",{{"Overdue ID", Int64.Type}}),
    #"Added Conditional Column1" = Table.AddColumn(#"Changed Type1", "Open Amount Positive ID", each if [OPEN_AMOUNT] > 1 then 1 else 0),
    #"Filtered Rows1" = Table.SelectRows(#"Added Conditional Column1", each Text.StartsWith([PROJECT_ID], "cp")),
    #"Added Custom" = Table.AddColumn(#"Filtered Rows1", "Overdue Open Amount", each if [Overdue ID] = 1 then [OPEN_AMOUNT] else 0)
in
    #"Added Custom"
```
