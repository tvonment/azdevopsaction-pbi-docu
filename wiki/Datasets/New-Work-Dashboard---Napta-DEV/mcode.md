...
|Dataset|[New Work Dashboard - Napta DEV](./../New-Work-Dashboard---Napta-DEV.md)|
| :--- | :--- |
...
var _countries = 
Union(
calculatetable(values('rls country project'[permission_to]), 'rls country project'[email] = userprincipalname())
,
Row("permission_to", _user_country)
)
...
var _projects = calculatetable(values(ParticipantsAndStaffing[ProjectNumber])
	, ParticipantsAndStaffing[ProjectNumber] = _pn
, IsBlank( ParticipantsAndStaffing[country_code_dm]) = false
	, Treatas(_countries, ParticipantsAndStaffing[country_code_dm])
	)
...
var _dmCountry = [country_code_dm]

return 
IsBlank(_dmCountry) = false
&&
(
_dmCountry = _user_country
||
(
calculate(CountRows('rls country project'), 'rls country project'[permission_to] = _dmCountry && 'rls country project'[email] = userprincipalname() ) > 0
)
)
```


### Country Only Project
...
&&
(
calculate(CountRows('rls countryOnly project'), 'rls countryOnly project'[permission_to] = _dmCountry && 'rls countryOnly project'[email] = userprincipalname() ) > 0
)
```
...
```m
var _countries = 
calculatetable(values('rls countryOnly project'[permission_to]), 'rls countryOnly project'[email] = userprincipalname())
