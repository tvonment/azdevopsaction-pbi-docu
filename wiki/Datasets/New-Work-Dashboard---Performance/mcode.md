...
|Dataset|[New Work Dashboard - Performance](./../New-Work-Dashboard---Performance.md)|
| :--- | :--- |
...
```m
let
    Source = vStaffingDetailsForNewWork,
    #"Removed Other Columns" = Table.SelectColumns(Source,{"ProjectName", "Client", "ProjectNumber", "ProjectEndDate", "EmployeeIDDM", "EmployeeIDPM"}),
    #"Removed Duplicates" = Table.Distinct(#"Removed Other Columns"),
    #"Appended Query" = Table.Combine({#"Removed Duplicates", #"No Project Assignment"}),
    #"Added Custom" = Table.AddColumn(#"Appended Query", "Project", each [ProjectNumber] & " - " & [Client]),
...
let
    Source = vStaffingDetailsForNewWork,
    #"Removed Other Columns" = Table.SelectColumns(Source,{"EmployeeID", "Subject", "StartDate", "EndDate", "DaysAssigned", "ProjectNumber", "StaffingInfo", "EmployeeIDDM", "EmployeeIDPM"})
in
    #"Removed Other Columns"
```
...
```m
let
    Source = Sql.Database("muc-mssql-5", "StaffMANGlobal"),
    dbo_vStaffingDetailsForNewWork = Source{[Schema="dbo",Item="vStaffingDetailsForNewWorkAll"]}[Data],
    #"Changed Type" = Table.TransformColumnTypes(dbo_vStaffingDetailsForNewWork,{{"EndDate", type date}, {"StartDate", type date}}),
    #"Filtered Rows" = Table.SelectRows(#"Changed Type", each [StartDate] <= _StaffingTo and [EndDate] >= _StaffingSince),
    Custom1 = Table.SelectColumns(#"Filtered Rows",{"EmployeeID", "Subject", "StartDate", "EndDate", "DaysAssigned", "ProjectNumber", "StaffingInfo", "EmployeeIDDM", "EmployeeIDPM"})
in
    Custom1
```
