...
```dax
size_Key = CONCATENATE('wipes_dim Wide'[Country],CONCATENATE("-",CONCATENATE('wipes_dim Wide'[BU],CONCATENATE("-",CONCATENATE('wipes_dim Wide'[Clorox Sub Category Value],CONCATENATE("-",'wipes_dim Wide'[Clorox Size Range Value]))))))
```
...
SWITCH (
    TRUE (),
    'wipes_dim Wide'[Country]
        IN { "HKG", "MYS", "SKR"}
        && 'wipes_dim Wide'[BU]
        IN { "BLEACH", "LA", "CW", "FP" }
        && 'wipes_dim Wide'[Clorox Brand Value] IN{ "Clorox","Glad", "Rox", "Yuhangen"}, "CLX",
    'wipes_dim Wide'[Country] IN{ "ARG","CHI","PAN","PER","PUR"}
        ...
        IN { "Ayudin","Trenet","Pinoluz","Clorinda", "Poett","Pine Sol","Mistolin","Clorox Clorogel","Lestoil" }, "CLX",
    'wipes_dim Wide'[Country] IN{ "CHI","COL","CRC","ECU","PAN","PER"}
        && 'wipes_dim Wide'[Clorox Sub Category Value]
        IN {
        "Color LA",
        "Disinfecting CW",
        ...
        && 'wipes_dim Wide'[Clorox Brand Value]
        IN {  "Clorox","Clorox Ropa Blanca","Clorox Cloro Gel","Limpido","Clorox Ropa Color" }, "CLX",
    'wipes_dim Wide'[Country] IN { "MEX"}
        && 'wipes_dim Wide'[Clorox Sub Category Value]
        IN {
        "Color LA",
        "Disinfecting CW",        
        ...
        && 'wipes_dim Wide'[Clorox Brand Value] IN { "Clorox", "Poett" }, "CLX",
    'wipes_dim Wide'[Country] IN { "PUR"}
        && 'wipes_dim Wide'[Clorox Sub Category Value]
        IN {
        "Color LA",
        "Disinfecting CW",  
        ...
        && 'wipes_dim Wide'[Clorox Brand Value] IN { "Clorox"}, "CLX",    
    'wipes_dim Wide'[Division] IN { "AMEA"}
        && 'wipes_dim Wide'[Clorox Brand Value] IN { "Clorox","Chux","Glad To Be Glad","Glad"},"CLX",
    'wipes_dim Wide'[Country] IN { "CND"}
       && 'wipes_dim Wide'[Clorox Brand Value] IN { "Clorox","Pine Sol","Brita"}, "CLX",
...
SWITCH(
    TRUE(),
    'wipes_dim Wide'[Clorox Size Range Value] in {"Small","Medium","Large","Giant","Mini"},
    CONCATENATE(
        'wipes_dim Wide'[Clorox Size Range Value],CONCATENATE(" (",
    CONCATENATE(
        'wipes_dim Wide'[Description],")"
        )
        )
        ),
...
```dax
Avg of $ / volume = CALCULATE(AVERAGE('wipes_val Wide'[$ / oz_All SKUs]),FILTER('wipes_val Wide','wipes_val Wide'[Year]= [selected_year]))
```
...
```dax
SP_Retailer rank = RELATED('Retailers Table'[Rank])
```
...
```dax
selected_year = MAX('date_table'[YEAR])
```
...
var en = iferror(CALCULATE( SUMX ( Table1, ( ( ( [mix] / [val2] ) * [val1] ) - [total volume] ) * [price/lb] )),0)
...
```dax
PP_Units = CALCULATE(SUM('wipes_val Wide'[Units]),FILTER('wipes_val Wide','wipes_val Wide'[Year]=[selected_year]))
```
...
```dax
PP_%Promo Units = DIVIDE([PP_Units],[PP_Total Units])
```



```dax
PP_Total Units = [PP_Units]
```
...
var b = CALCULATE(SUM('wipes_val Wide'[Units]), FILTER('wipes_val Wide','wipes_val Wide'[Year]= [py_year]))
...
CONCATENATE("L",
             CONCATENATE(
                 Mid(date_table[Refreshed Period],8,2),
                     CONCATENATE(" Months-",
                        RIGHT(date_table[Refreshed Period],8)
                        )
                        )
                        )
```
...
VAR en = IF ( [py_year] = [selected_year] - 2, ( - val3 + val4 ), ( - val1 + val2 ) )
...
PSA_Volume Share CY = 
var dollar = CALCULATE(SUM('wipes_val Wide'[Volume Sales]),FILTER('wipes_val Wide','wipes_val Wide'[Year]=[selected_year]))
var deno = CALCULATE([PSA_Total volume share_cy],ALL('wipes_dim Wide'[Clorox Brand Value]),ALL('wipes_dim Wide'[Clorox Size Range Value]),ALL('wipes_dim Wide'[Clorox Size Value], 'wipes_dim Wide'[Size Calc],'wipes_dim Wide'[Index size range],'wipes_dim Wide'[Range_description]))
...
VAR MonthlyTable = 
    ADDCOLUMNS(
        SUMMARIZE(
            'wipes_val Wide',
            'wipes_dim Wide'[Clorox Brand Value],'wipes_dim Wide'[Product Key],'wipes_dim Wide'[Clorox Size Range Value],'wipes_dim Wide'[Size Calc],'wipes_dim Wide'[Range_description]
        ),
        "$/oz", [lc$/oz],
        //"$/unit",[lc$/unit],
        "revenue", [lc Total Revenue]
...
var deno = CALCULATE([PSA_Total volume share_py],ALL('wipes_dim Wide'[Clorox Brand Value]),ALL('wipes_dim Wide'[Clorox Size Range Value]),ALL('wipes_dim Wide'[Clorox Size Value], 'wipes_dim Wide'[Size Calc],'wipes_dim Wide'[Index size range],'wipes_dim Wide'[Range_description]))
...
```dax
Executive header = "NRM - Category Performance ( " & SELECTEDVALUE('wipes_dim Wide'[Country-Name])&" )"
```
...
% Volume = 
var dollar = CALCULATE(SUM('wipes_val Wide'[Volume Sales]),FILTER('wipes_val Wide','wipes_val Wide'[Year]=[selected_year]))
var deno = CALCULATE([PSA_Total volume share_cy],ALL('wipes_dim Wide'[Clorox Size Range Value]),ALL('wipes_dim Wide'[Clorox Size Value], 'wipes_dim Wide'[Size Calc],'wipes_dim Wide'[Index size range],'wipes_dim Wide'[Range_description]))

return
CONCATENATE(CONCATENATE(FORMAT ( ABS(ROUND([PSA_Total volume share_cy],1)), "#,##0,,.0M" )
     ,
    CONCATENATE (" (",
        FORMAT (DIVIDE(dollar,deno,0) , "0.0%" ))
...
```dax
lc $/oz WAvg( brand ) = 
DIVIDE([lc $/oz wavg num (brand)]
    ,
        CALCULATE([lc Total Revenue],ALL('wipes_dim Wide'[Clorox Size Range Value],'wipes_dim Wide'[Index size range],'wipes_dim Wide'[Range_description])),0)
```



```dax
lc $/oz wavg num (brand) = 
 CALCULATE(SUMX(
        'wipes_dim Wide',Measure_Table2[lc$/oz] *Measure_Table2[lc Total Revenue]),ALL('wipes_dim Wide'[Clorox Size Range Value],'wipes_dim Wide'[Index size range],'wipes_dim Wide'[Range_description]))
```



```dax
total revenue e.v_matrix = CONCATENATE("Total Revenue : ",CONCATENATE( CONCATENATE( IF([$ cy]<0,"-$ ","+$ "), FORMAT ( ABS(ROUND([$ cy],1)), "#,##0,,.0M" ))," (" & FORMAT ( [Revenue_%], "0.0%" ) &")"
...
```dax
Swot_X Axis Lable = "Impact of driver("&SELECTEDVALUE('Revenue Drivers'[Drivers])&") for " & SELECTEDVALUE('wipes_dim Wide'[Clorox Brand Value])
...
SELECTEDVALUE ( swot_data_nr[Clorox Brand Value] ) & " Leads "
    &
                    SELECTEDVALUE ( 'wipes_dim Wide'[Clorox Brand Value] )& " Lags"
```
...
"Both " & SELECTEDVALUE ( swot_data_nr[Clorox Brand Value] ) & " and "
    &  SELECTEDVALUE ( 'wipes_dim Wide'[Clorox Brand Value] ) & " Lead"
```
...
"Both " & SELECTEDVALUE ( swot_data_nr[Clorox Brand Value] ) & " and "
    & 
                    SELECTEDVALUE ( 'wipes_dim Wide'[Clorox Brand Value] )
                & " Lag"
```
...
CALCULATE([Cat_Swot_Selected Driver], FILTER(ALL('wipes_dim Wide'[Clorox Brand Value]), 'wipes_dim Wide'[Clorox Brand Value]=SELECTEDVALUE(swot_data_nr[Clorox Brand Value]))))
...
IF(HASONEVALUE(swot_data_nr[Clorox Brand Value]),
CALCULATE([Driver_Swot_Selected Driver], FILTER(ALL('wipes_dim Wide'[Clorox Brand Value]), 'wipes_dim Wide'[Clorox Brand Value]=SELECTEDVALUE(swot_data_nr[Clorox Brand Value]))))
```
...
```dax
Title Lable for Add Pricing analysis = "Pricing Analysis " &SELECTEDVALUE('wipes_dim Wide'[Clorox Brand Value])
```
...
var dollar = CALCULATE(SUM('wipes_val Wide'[Volume Sales]),FILTER('wipes_val Wide','wipes_val Wide'[Year]=[selected_year]))
var deno = CALCULATE([PSA_Total volume share_cy],ALL('Brand Ranking'[Clorox Brand Value]),ALL('wipes_dim Wide'[Clorox Brand Value],'wipes_dim Wide'[Clorox Size Range Value],'wipes_dim Wide'[Index size range],'wipes_dim Wide'[Range_description]))
    ...
    IF (
        ...
        RETURN
            IF (
                ...
                VAR VisibleProducts =
                    CALCULATETABLE (
                        SUMMARIZE('wipes_dim Wide','wipes_dim Wide'[Clorox Brand Value],'wipes_dim Wide'[Clorox Size Range Value],'wipes_dim Wide'[Index size range],'wipes_dim Wide'[Range_description],"volumeshare",[Vol Share for NRM PSD])
...
var dollar = CALCULATE(SUM('wipes_val Wide'[Volume Sales]),FILTER('wipes_val Wide','wipes_val Wide'[Year]=[py_year]))
var deno = CALCULATE([PSA_Total volume share_py],ALL('Brand Ranking'[Clorox Brand Value]),ALL('wipes_dim Wide'[Clorox Brand Value],'wipes_dim Wide'[Clorox Size Range Value],'wipes_dim Wide'[Index size range],'wipes_dim Wide'[Range_description]))
...
RETURN
 ...
 VAR VisibleProducts =
 CALCULATETABLE (
 SUMMARIZE('wipes_dim Wide','wipes_dim Wide'[Clorox Brand Value],'wipes_dim Wide'[Clorox Size Range Value],'wipes_dim Wide'[Index size range],'wipes_dim Wide'[Range_description],"volumeshare",[Vol Share for NRM PSD test])
...
```dax
Size discription = CONCATENATE( SELECTEDVALUE(Pack_Size_Range_description[Pack Size Range]),CONCATENATE(" : ",SELECTEDVALUE(Pack_Size_Range_description[Description])))
```



```dax
Size discription 1 = CONCATENATE( SELECTEDVALUE(Pack_Size_Range_description[Pack Size Range]),CONCATENATE(" : ",SELECTEDVALUE(Pack_Size_Range_description[Description])))
```
...
```dax
SUMMARIZE('wipes_val Wide','wipes_val Wide'[Retailer],
"Rank",SWITCH(
    TRUE(),'wipes_val Wide'[Retailer]="MT",1,
             'wipes_val Wide'[Retailer]="TT",2,
                'wipes_val Wide'[Retailer]="Total",3))
```
